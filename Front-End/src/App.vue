<template>
  <div class="container">
    <nav class="navbar">
      ...
      <div class="nav-links">
        <button v-if="!isLoggedIn()" @click="state.showLogin = true">Iniciar Sesión</button>
        <button v-if="!isLoggedIn()" @click="state.showRegister = true">Registrarse</button>
        <span v-if="isLoggedIn()">Bienvenido, {{ state.user ? state.user.rol : '' }}</span>
      ...
      <div class="modal-content">
        ...
        <form @submit.prevent="login">
          <div class="form-group">
            <label>Email</label>
            <input v-model="state.loginData.email" required placeholder="Ej: usuario@ejemplo.com">
          </div>
          <div class="form-group">
            <label>Contraseña</label>
            <input v-model="state.loginData.password" type="password" required placeholder="Contraseña">
          </div>
          <button type="submit">Iniciar Sesión</button>
          <button type="button" @click="state.showLogin = false">Cancelar</button>
        </form>
      </div>
    </div>

    <div v-if="state.showRegister" class="modal">
      <div class="modal-content">
        <h2>Registrarse</h2>
        <form @submit.prevent="register">
          <div class="form-group">
            <label>Email</label>
            <input v-model="state.registerData.email" required placeholder="Ej: usuario@ejemplo.com">
          </div>
          <div class="form-group">
            <label>Contraseña</label>
            <input v-model="state.registerData.password" type="password" required placeholder="Contraseña">
          </div>
          <button type="submit">Registrarse</button>
          <button type="button" @click="state.showRegister = false">Cancelar</button>
        </form>
      </div>
    </div>
    ...
    <div v-if="isLoggedIn()" class="main-content">
      <div v-if="isAdmin()" class="form-container">
        <h2>{{ state.isEditing ? 'Editar Videojuego' : 'Añadir Videojuego' }}</h2>
        <form @submit.prevent="guardarVideojoc">
          <div class="form-group">
            <label>Nombre</label>
            <input v-model="state.videojocActual.nombre" required placeholder="Ej: Elden Ring">
          </div>
          <div class="form-group">
            <label>URL de la Imagen</label>
            <input v-model="state.videojocActual.imagenUrl" required placeholder="Ej: https://example.com/image.jpg">
          </div>
          <div class="form-group">
            <label>Descripción</label>
            <textarea v-model="state.videojocActual.descripcion" required placeholder="Describe el videojuego"></textarea>
          </div>
          <div class="form-group">
            <label>URL del Sitio</label>
            <input v-model="state.videojocActual.sitioUrl" required placeholder="Ej: https://www.eldenring.com">
          </div>
          <button type="submit">{{ state.isEditing ? 'Actualizar' : 'Añadir' }}</button>
          <button type="button" v-if="state.isEditing" @click="resetFormulari">Cancelar</button>
        </form>
      </div>
  ...
  <div class="buttons">
    <button @click="state.showLogin = true">Iniciar Sesión</button>
    <button @click="state.showRegister = true">Registrarse</button>
  </div>
</div>
  </div>
</template>
